  ...
  try {
    ...
    const { text } = await generateText({
      ...
      system:
        ...
        "- account, document, filing, compliance, application, renewal, payment and appointment notices/reminders a business sends to its own customers or members (e.g. 'Your Compliance Report is still pending', 'Filing step required', 'Action needed on your application', 'File now: <link>'), including urgent-sounding but truthful wording and a link to the business's own site\n" +
        "- general service businesses advertising quality guarantees, honest times, and customer service\n\n" +
        "CRITICAL DISTINCTIONS:\n" +
        "- Urgency alone is NOT phishing or fraud. Only return phishing/fraud_deceptive when the message asks for credentials, card/bank/SSN details, one-time codes, or impersonates a government agency, bank, courier, or well-known brand it clearly is not, or promises a fake prize/refund. A notice telling the recipient a filing/report/document is pending and linking to a site to complete it is legitimate business communication.\n" +
        ...
        "- \"{{first_name}}: Filing Step Required. Your Compliance Report is still pending. File Now: https://example.com/home STOP to opt out\" (business compliance/filing reminder)\n" +
