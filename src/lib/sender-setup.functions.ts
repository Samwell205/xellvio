...
export const setupSms = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: acct, error } = await supabase
      .from("accounts")
      .select("id,legal_business_name,business_address,business_reg_number,website_url,privacy_policy_url,contact_email,full_name,phone,twilio_subaccount_sid,twilio_subaccount_auth_token_enc,onboarding_status")
      .eq("id", userId)
      .maybeSingle();
    ...
    for (const cc of data.targetCountries) {
      ...
      if (!num) { created.push(`${cc}:no_numbers`); continue; }
  ...
  for (const sa of pending) {
    ...
    if (creds === undefined) {
      const { data: acct } = await supabaseAdmin.from("accounts")
        .select("twilio_subaccount_sid,twilio_subaccount_auth_token_enc,contact_email,legal_business_name")
    ...
    try {
      const ver = await twilio<any>(`${MESSAGING_API}/Tollfree/Verifications/${sa.verification_sid}`, {
      ...
      if (mapped === "verified") {
        await supabaseAdmin.from("accounts").update({ onboarding_status: "active" }).eq("id", sa.account_id);
      }
      updated++;
    } catch {
