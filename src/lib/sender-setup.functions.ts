...
export const getMySenderAssets = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data } = await context.supabase
      .from("sender_assets")
      .select("id,country_code,sender_kind,phone_number,telnyx_messaging_profile_id,verification_status,rejection_reason,friendly_rejection_reason,telnyx_verification_id,submitted_at,in_review_at,verified_at,rejected_at,last_synced_at,telnyx_phone_number_id,telnyx_messaging_profile_id")
