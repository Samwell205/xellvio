...
import { CheckCircle2, AlertTriangle, Shield, ShieldCheck, Clock, X, ArrowRight, KeyRound, Mail, User } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/app/settings")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Manage your Xellvio profile, login email, password and SMS sender settings." },
      { property: "og:title", content: "Settings — Xellvio" },
      { property: "og:description", content: "Manage your Xellvio profile, login email, password and SMS sender settings." },
...
function TollfreeStatusCard() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["tollfree-verification"], queryFn: () => load() });
  ...
  let badge = <Badge variant="outline" className="gap-1"><Clock className="size-3" /> Not started</Badge>;
  let blurb = "Required to send SMS to US and Canadian recipients. Skip if you only send elsewhere.";
  if (status === "verified" || alt) {
    badge = <Badge className="gap-1 bg-emerald-500 text-white hover:bg-emerald-500"><CheckCircle2 className="size-3" /> Approved</Badge>;
    ...
    badge = <Badge className="gap-1 bg-blue-500 text-white hover:bg-blue-500"><Clock className="size-3" /> In review</Badge>;
  ...
  return (
    <Card className="space-y-3 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h3 className="flex items-center gap-2 font-semibold"><ShieldCheck className="size-4" /> Toll-free verification (US/Canada)</h3>
        {isLoading ? <Badge variant="outline">Loading…</Badge> : badge}
      </div>
      <p className="text-sm text-muted-foreground">{blurb}</p>
      <Button asChild variant="outline" size="sm">
        <Link to="/app/toll-free-verification">{status ? "Open verification" : "Start verification"} <ArrowRight className="ml-1 size-3.5" /></Link>
      </Button>
    </Card>
  );
}
  ...
  return (
    <Card className="space-y-4 p-6">
      <div className="space-y-1">
        <h3 className="flex items-center gap-2 font-semibold"><Mail className="size-4" /> Login email</h3>
        <p className="text-sm text-muted-foreground">This is the email you sign in with. We will ask you to confirm the new address.</p>
      </div>
...
function PasswordCard() {
  ...
  return (
    <Card className="space-y-4 p-6">
      <div className="space-y-1">
        <h3 className="flex items-center gap-2 font-semibold"><KeyRound className="size-4" /> Login password</h3>
        <p className="text-sm text-muted-foreground">Choose something at least 8 characters long that you do not use elsewhere.</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Button disabled={!current || !next || !confirm || change.isPending} onClick={() => change.mutate()}>
          {change.isPending ? "Saving…" : "Update password"}
        </Button>
        <Link to="/forgot-password" className="text-sm text-muted-foreground underline">Forgot your current password?</Link>
      </div>
    </Card>
  );
}
...
function SettingsPage() {
  ...
  const account = useQuery({
    queryKey: ["account"],
    queryFn: async () => (await supabase.from("accounts").select("id,email,contact_email,full_name,company,phone").maybeSingle()).data,
  });
  ...
  const save = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["account"] }); toast.success("Saved"); },
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">Your profile, sign-in details and sending setup.</p>
      </div>
      ...
      {tab === "personal" && (
        <div className="space-y-6">
          <Card className="space-y-4 p-6">
            <div className="space-y-1">
              <h3 className="flex items-center gap-2 font-semibold"><User className="size-4" /> Your details</h3>
              <p className="text-sm text-muted-foreground">Used on invoices and in the messages we send you.</p>
            </div>
            <div className="grid max-w-2xl gap-4 md:grid-cols-2">
              ...
              <div className="space-y-1.5"><Label>Login email</Label><Input value={account.data?.email ?? ""} disabled /></div>
            </div>
        ...
        <div className="space-y-6">
          <Card className="space-y-3 p-6">
            ...
            <div className="flex flex-wrap gap-2">
              <Button asChild variant="outline" size="sm"><Link to="/app/team">Manage team</Link></Button>
              <Button asChild variant="outline" size="sm"><Link to="/app/billing">Billing &amp; credits</Link></Button>
            </div>
          </Card>
          {isAdmin.data && (
            <Card className="space-y-3 p-6">
              <h3 className="flex items-center gap-2 font-semibold"><Shield className="size-4" /> Admin access</h3>
              <div className="flex items-center gap-2 text-sm text-success">
                <CheckCircle2 className="size-4" />
                You have admin privileges.
              </div>
              <Button asChild variant="outline" size="sm"><Link to="/admin/users">Manage users &amp; roles</Link></Button>
            </Card>
          )}
        </div>
      )}
      ...
      {tab === "messaging" && (
        <div className="space-y-6">
          ...
          <Card className="space-y-3 p-6">
            ...
            <div className="flex items-center gap-2 text-sm"><CheckCircle2 className="size-4 text-success" /> Sending connection active.</div>
            <div className="flex items-center gap-2 text-sm">
              <AlertTriangle className="size-4 text-warning" />
              Your sender is matched to each recipient's country automatically.
            </div>
            <Button asChild variant="outline" size="sm"><Link to="/app/setup-sms">Open SMS setup</Link></Button>
          </Card>
          <Card className="space-y-3 p-6 opacity-90">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h3 className="font-semibold">Gorgias helpdesk</h3>
              <Badge variant="outline" className="gap-1"><Clock className="size-3" /> Coming soon</Badge>
            </div>
            <p className="text-sm text-muted-foreground">
