...
export const Route = createFileRoute("/_authenticated/app/developer")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Create, test, submit and monitor your Xellvio marketplace integrations.",
      },
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Publish an integration to every Xellvio workspace — actions, triggers, webhooks and
            analytics.
          </p>
        </div>
              ...
              {(overview.data?.apps ?? []).map((app: any) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    {app.review_notes && (
                      <p className="mt-1 text-xs text-destructive">Review: {app.review_notes}</p>
                    )}
                  </div>
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-base">
          {dev ? "Developer profile" : "Create your developer profile"}
        </CardTitle>
      </CardHeader>
      ...
      {rows.map((r, i) => (
        <div key={i} className="grid gap-2 sm:grid-cols-[1fr_1fr_150px]">
          <Input
            ...
            value={r.name}
            onChange={(e) =>
              set(rows.map((x, j) => (j === i ? { ...x, name: e.target.value } : x)))
            }
          />
          <Input
            ...
            value={r.description ?? ""}
            onChange={(e) =>
              set(rows.map((x, j) => (j === i ? { ...x, description: e.target.value } : x)))
            }
          />
          <Input
            ...
            value={r.entity ?? ""}
            onChange={(e) =>
              set(rows.map((x, j) => (j === i ? { ...x, entity: e.target.value } : x)))
            }
          />
        </div>
      ))}
      ...
      <CardContent className="grid max-w-3xl gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <Textarea
            ...
            onChange={(e) => setV({ ...v, longDescription: e.target.value })}
          />
        </div>
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="space-y-1.5">
            ...
            <Select
              value={v.authType}
              onValueChange={(x) => setV({ ...v, authType: x as typeof v.authType })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={v.pricingType}
              onValueChange={(x) => setV({ ...v, pricingType: x as typeof v.pricingType })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-1.5">
            ...
            <Input
              value={v.baseApiUrl}
              onChange={(e) => setV({ ...v, baseApiUrl: e.target.value })}
          ...
          <div className="space-y-1.5">
            ...
            <Input
              value={v.webhookUrl}
              onChange={(e) => setV({ ...v, webhookUrl: e.target.value })}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button
            ...
            size="sm"
            onClick={() => test.mutate()}
            disabled={!v.baseApiUrl || test.isPending}
          >
            Test connection
          </Button>
        ...
        <div className="space-y-1.5">
          ...
          <Textarea
            rows={3}
            value={v.setupGuide}
            onChange={(e) => setV({ ...v, setupGuide: e.target.value })}
          />
        </div>
        ...
        <Button
          className="w-fit"
          onClick={() => save.mutate()}
          disabled={save.isPending || v.name.trim().length < 2}
        >
          Save draft
        </Button>
      </CardContent>
