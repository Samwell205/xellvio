  ...
  if (profile && !profile.verifier) {
    return (
      <div className="max-w-lg space-y-4">
        ...
        <Link to="/verify/dashboard/settings">
          <Button>Set up bank details</Button>
        </Link>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">
          Welcome{profile?.verifier ? `, ${profile.verifier.full_name.split(" ")[0]}` : ""}
        </h1>
        ...
        <Card className="bg-slate-900 border-slate-800">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ₦{Number(profile?.wallet?.balance_ngn ?? 0).toLocaleString()}
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-900 border-slate-800">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ₦{Number(profile?.wallet?.lifetime_earned_ngn ?? 0).toLocaleString()}
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-900 border-slate-800">
      ...
      <div className="flex gap-3">
        <Link to="/verify/dashboard/numbers">
          <Button>Submit a number</Button>
        </Link>
        <Link to="/verify/dashboard/withdrawals">
