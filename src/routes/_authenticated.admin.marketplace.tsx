    ...
    <Card className="mt-4">
      <CardContent className="p-0">
        <div className="divide-y">
          {(q.data ?? []).map((r: any) => (
            <div key={r.id} className="p-3 grid grid-cols-6 gap-2 text-sm items-center">
              ...
              <div className="tabular-nums">
                {r.buyer_price_amount ? `$${Number(r.buyer_price_amount).toFixed(2)}` : "—"}
              </div>
          ...
          {!q.data?.length && (
            <div className="p-6 text-sm text-muted-foreground">No listings yet.</div>
          )}
        </div>
      </CardContent>
    </Card>
...
function Withdrawals() {
  ...
  const mark = useMutation({
    mutationFn: (v: { id: string; status: "paid" | "rejected"; notes?: string }) =>
      markFn({ data: v }),
    onSuccess: () => {
    ...
    <Card className="mt-4">
      <CardContent className="p-0">
        <div className="divide-y">
              ...
              <div key={r.id} className="p-3 space-y-2 text-sm">
                ...
                {r.status === "pending" && (
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => mark.mutate({ id: r.id, status: "paid" })}>
                      Mark paid
                    </Button>
                    <Button
                      size="sm"
          ...
          {!q.data?.length && (
            <div className="p-6 text-sm text-muted-foreground">No withdrawal requests.</div>
          )}
        </div>
      </CardContent>
    </Card>
...
function Pricing() {
  ...
  const seeded =
    q.data && buyer === ""
      ? (setBuyer(String(q.data.buyerPrice)),
        setPayout(String(q.data.sellerPayout)),
        setFee(String(q.data.verificationFee)),
        true)
      : false;
  void seeded;
  return (
    <Card className="mt-4">
      <CardHeader>
        <CardTitle className="text-base">Marketplace pricing</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3 max-w-md">
        <div>
          <Label>Buyer price (USD)</Label>
          <Input value={buyer} onChange={(e) => setBuyer(e.target.value)} inputMode="decimal" />
        </div>
        <div>
          <Label>Seller payout (USD)</Label>
          <Input value={payout} onChange={(e) => setPayout(e.target.value)} inputMode="decimal" />
        </div>
        <div>
          <Label>Seller verification fee (USD)</Label>
        ...
        <Button
          onClick={async () => {
            try {
              await setFn({
                data: {
                  buyerPrice: Number(buyer),
                  sellerPayout: Number(payout),
                  verificationFee: Number(fee),
                },
              });
