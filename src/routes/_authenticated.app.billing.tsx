...
function BillingPage() {
    ...
    queryFn: async () =>
      (
        await supabase
          .from("accounts")
          .select(
            "id,credit_balance,auto_recharge_enabled,auto_recharge_threshold,auto_recharge_amount,contact_email,email",
          )
          .maybeSingle()
      ).data,
  });
    ...
    queryFn: async () =>
      (
        await supabase
          ...
          .select("id,type,amount,balance_after,description,created_at,campaign_id")
  ...
  const spend30d = (tx.data ?? [])
    .filter(
      (t) => t.type === "debit" && new Date(t.created_at).getTime() > Date.now() - 30 * 86400_000,
    )
    ...
    <div className="mx-auto max-w-6xl space-y-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Your balance, payments and top-up preferences.
        </p>
      </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <Card className="p-6">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Credits are used as your messages go out.
              </p>
            </Card>
            <Card className="p-6">
              <div className="text-xs uppercase tracking-wide text-muted-foreground">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Across all campaigns and replies.
              </p>
            </Card>
            <Card className="p-6">
              <div className="text-xs uppercase tracking-wide text-muted-foreground">
              ...
              <div className="mt-1 text-lg font-semibold capitalize">
                {lastPaid?.provider ?? "Card or crypto"}
              </div>
              <p className="mt-2 text-xs text-muted-foreground">
                {lastPaid
                  ? `Last payment ${new Date(lastPaid.created_at).toLocaleDateString()}`
                  : "Choose at checkout — nothing is stored here."}
              </p>
            </Card>
          </div>
          ...
          <Card className="space-y-4 p-6">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                Priced in USD · pay by card or crypto
              </span>
            </div>
            <PackPicker packs={usdPacks.filter((p) => Number(p.price) <= 500)} />
            <p className="text-xs text-muted-foreground">
              Messages are billed per recipient at the country rate × segments. We never debit more
              than your available balance — anything your balance can't cover is skipped, not
              charged.
            </p>
          </Card>
        </div>
                  ...
                  <tbody>
                    {tx.data!.map((t) => (
                      <tr key={t.id} className="border-t">
                        ...
                        <td
                          className={`p-3 text-right font-medium tabular-nums ${t.type === "topup" || t.type === "refund" ? "text-success" : "text-destructive"}`}
                        >
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setAuto({ ...auto, threshold: Number(e.target.value) })}
            ...
            <div>
              ...
              <Input
                ...
                min={1}
                value={auto.amount}
                onChange={(e) => setAuto({ ...auto, amount: Number(e.target.value) })}
                disabled={!auto.enabled}
              />
            </div>
          </div>
          <Button onClick={() => saveAuto.mutate()} disabled={saveAuto.isPending} variant="outline">
            Save auto top-up
          </Button>
        </Card>
      )}
...
function PackPicker({ packs }: { packs: any[] }) {
  ...
  if (!packs.length) {
    return (
      <p className="text-sm text-muted-foreground">
        No packs available. Ask the admin to create one.
      </p>
    );
  }
            ...
            <SelectContent>
              {packs.map((p) => (
                <SelectItem key={p.id} value={p.id}>
                  {p.name} — {formatUSD(Number(p.price))} ({formatUSD(Number(p.credits))} credits)
        ...
        {isCustom && (
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              1 USD = 1 credit · min $5, max $10,000
            </p>
          </div>
        )}
        {amount < 25 && (
          <div className="rounded-md border border-amber-500/40 bg-amber-50 dark:bg-amber-950/30 p-3 text-xs text-amber-800 dark:text-amber-200">
            <strong>Heads up:</strong> Crypto payments (USDT, BTC, etc.) usually require at least{" "}
            <strong>$25</strong> to clear the network's minimum. For amounts under $25, please pay
            by <strong>card</strong> — or pick the Growth ($25) pack or higher for crypto.
          </div>
        )}
      </div>
