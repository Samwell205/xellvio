          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Sellers verify toll-free numbers and get paid when businesses buy them. Confirm to
              ...
              <Button
                onClick={async () => {
                  ...
                  qc.invalidateQueries({ queryKey: ["seller-status"] });
                }}
              >
  ...
  return (
    <Shell>
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <Stat
          label="Buyer price / your payout"
          value={`$${status.data.pricing.buyerPrice} → $${status.data.pricing.sellerPayout}`}
        />
      </div>
      <Tabs defaultValue="numbers" className="mt-6">
        ...
        <TabsContent value="withdrawals">
          <Withdrawals balance={status.data.balance} />
        </TabsContent>
...
function MyNumbers() {
  ...
  return (
    <Card className="mt-4">
      <CardHeader>
        <CardTitle className="text-base">My numbers</CardTitle>
      </CardHeader>
      <CardContent>
        {!q.data?.length ? (
          <div className="text-sm text-muted-foreground">
            No numbers yet. Verify a toll-free number in the tenant app under{" "}
            <Link to="/app/toll-free-verification" className="underline">
              Toll-free verification
            </Link>
            . Once approved, it appears here and becomes available on the marketplace.
          </div>
        ) : (
            ...
            {q.data.map((l: any) => (
              <div key={l.id} className="py-2 flex items-center gap-3 text-sm">
                ...
                <div className="ml-auto text-muted-foreground">
                  {l.sold_at
                    ? `Sold ${new Date(l.sold_at).toLocaleDateString()}`
                    : new Date(l.created_at).toLocaleDateString()}
                </div>
                {l.seller_payout_amount && (
                  <div className="tabular-nums font-medium">
                    +${Number(l.seller_payout_amount).toFixed(2)}
                  </div>
                )}
              </div>
            ))}
    ...
    <Card className="mt-4">
      ...
      <CardContent className="space-y-4">
        {current.data && (
          <div className="rounded-md border bg-muted/40 p-3 text-sm">
            <div>
              ...
              <strong>{current.data.account_name}</strong>
            </div>
            <div className="text-muted-foreground">
              {current.data.bank_name} • {current.data.account_number}
            </div>
          </div>
        )}
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            disabled={!bankCode || accNum.length !== 10}
            onClick={async () => {
              try {
                const r = await resolveFn({
                  data: { bank_code: bankCode, account_number: accNum },
                });
                setResolved(r);
              } catch (e: any) {
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                await saveFn({
                  data: { bank_code: bankCode, bank_name: bankName, account_number: accNum },
                });
...
function Withdrawals({ balance }: { balance: number }) {
  ...
  const mut = useMutation({
    ...
    onSuccess: () => {
      toast.success("Withdrawal requested — admin will process it manually.");
      setAmount("");
    ...
    <div className="mt-4 space-y-4">
      <Card>
        ...
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Available: <strong className="text-foreground">${balance.toFixed(2)}</strong> • Minimum
            $5
          </p>
          <div className="flex gap-2">
            <Input
            ...
            <div className="divide-y">
              {q.data.map((w: any) => (
                <div key={w.id} className="py-2 text-sm flex items-center gap-3">
                  ...
                  {w.admin_notes && (
                    <div className="text-muted-foreground italic">— {w.admin_notes}</div>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="divide-y">
            {q.data.map((l: any) => (
              <div key={l.id} className="py-2 text-sm flex items-center gap-3">
                ...
                <div
                  className={`tabular-nums font-medium ${Number(l.amount) < 0 ? "text-destructive" : "text-emerald-600"}`}
                >
