...
function AdminTelnyxPage() {
  ...
  const spend = useQuery({
    queryKey: ["admin-telnyx-spend"],
    queryFn: () => spendFn(),
    refetchInterval: 60_000,
  });
  const bal = useQuery({
    queryKey: ["admin-telnyx-live"],
    queryFn: () => balFn(),
    refetchInterval: 60_000,
  });
    ...
    <div className="space-y-6 max-w-7xl">
      <div>
        <h1 className="text-2xl font-extrabold flex items-center gap-2">
          <Phone className="size-6" /> Telnyx activity
        </h1>
        <p className="text-sm text-muted-foreground">
          Live balance, every dollar spent, and what each campaign cost you at Telnyx.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <Card className="p-5">
          ...
          <Button asChild size="sm" variant="outline" className="mt-3">
            <a
              href="https://portal.telnyx.com/#/app/billing/transactions"
              target="_blank"
              rel="noreferrer"
            >
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-semibold flex items-center gap-2">
            <TrendingDown className="size-4" /> Where the balance went
          </h3>
          <Button
            size="sm"
            variant="ghost"
          ...
          >
            <RefreshCw className={`size-3.5 mr-1.5 ${spend.isFetching ? "animate-spin" : ""}`} />{" "}
            Refresh
          </Button>
        </div>
        {spend.data && spend.data.implied_spend_from_snapshots !== null && (
          <p className="text-sm text-muted-foreground">
            ...
            <strong className="text-foreground">
              {formatUSD(spend.data.implied_spend_from_snapshots)}
            </strong>
            . Our messages table accounts for{" "}
            <strong className="text-foreground">
              {formatUSD(spend.data.windows.last_30d.telnyx_cost)}
            </strong>{" "}
            of Telnyx carrier cost in the same window. The gap (if any) is number rentals,
            verification fees, or MMS not billed through campaigns.
          </p>
        )}
              ...
              {(spend.data?.windows.last_30d.by_country ?? []).map((c) => (
                <tr key={c.country} className="border-t">
                  ...
                  <td className="p-3 text-right tabular-nums text-destructive">
                    {formatUSD(c.telnyx_cost)}
                  </td>
                  <td className="p-3 text-right tabular-nums text-success">
                    {formatUSD(c.tenant_spend)}
                  </td>
                  <td className="p-3 text-right tabular-nums font-medium">
                    {formatUSD(c.tenant_spend - c.telnyx_cost)}
                  </td>
                </tr>
              ))}
      ...
      <Card className="p-5 space-y-2">
        <ul className="text-sm space-y-1 list-disc pl-5 text-muted-foreground">
          <li>
            ...
            — every top-up and every debit line item.
          </li>
          <li>
            <a
              className="underline text-primary"
              href="https://portal.telnyx.com/#/app/reporting/messaging"
              target="_blank"
              rel="noreferrer"
            >
              Reporting → Messaging
            </a>{" "}
            — per-message MDRs with cost and destination.
          </li>
          <li>
            <a
              className="underline text-primary"
            ...
            — monthly number rentals (these debit even with zero SMS).
          </li>
        </ul>
        <p className="text-xs text-muted-foreground">
          If Telnyx Transactions shows charges you don't see above, it's almost always number
          rentals or verification fees — not SMS.
        </p>
      </Card>
      ...
      <Card className="p-5 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          US MMS is now priced at <strong>$0.048/message</strong> (Telnyx cost $0.024 + $0.024
          margin). A retroactive charge of <strong>$0.024 × successful MMS</strong> has been applied
          to <strong>PRINCESS POLLY</strong> (emmanuelolushola824@gmail.com). Click below to email
          the tenant the explanation.
        </p>
        <Button
          size="sm"
          onClick={async () => {
            try {
              const res = await sendMmsCorrectionEmail({
                data: { accountId: "73d366b2-d9e0-4fb3-8635-a3707505ced0" },
              });
              if (res.ok)
                toast.success(
                  `Email sent — $${res.additional_charged} debited for ${res.message_count} MMS. Balance $${res.balance.toFixed(2)}`,
                );
...
function SpendBox({ label, v }: { label: string; v: any }) {
  return (
    <Card className="p-5">
      ...
      <div className="text-3xl font-extrabold tabular-nums mt-1 text-destructive">
        {v ? formatUSD(v.telnyx_cost) : "—"}
      </div>
      <div className="text-xs text-muted-foreground mt-1">
...
function BalanceTrend({
  ...
  snapshots: Array<{ balance: number; checked_at: string; status: string }>;
}) {
  if (!snapshots.length)
    return <div className="text-sm text-muted-foreground">No balance snapshots yet.</div>;
...
function DailySpend({
  ...
  rows: Array<{ day: string; telnyx_cost: number; segments: number }>;
}) {
  const max = Math.max(1, ...rows.map((r) => r.telnyx_cost));
  return (
    <div>
      <div className="text-xs uppercase tracking-wide text-muted-foreground mb-1">
        Daily Telnyx spend (14d)
      </div>
      <div className="flex items-end gap-1 h-24">
        {rows.map((r) => (
          <div
            ...
            title={`${r.day}: ${formatUSD(r.telnyx_cost)}`}
          >
            <div
              className="w-full bg-destructive/70 rounded-t"
              style={{
                height: `${(r.telnyx_cost / max) * 100}%`,
                minHeight: r.telnyx_cost > 0 ? 2 : 0,
              }}
            />
          </div>
        ))}
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center justify-between mb-1">
        <h3 className="font-semibold flex items-center gap-2">
          <Wallet className="w-4 h-4 text-primary" /> Where the wallet money went — by outcome (30d)
        </h3>
      </div>
      <p className="text-xs text-muted-foreground mb-4">
        Every message the carrier accepted is billed the moment it hits the network, even if it
        never delivers. That's why your wallet drops faster than the "delivered" count suggests.
      </p>

      <div className="grid md:grid-cols-4 gap-3 mb-4">
        <Stat label="Wallet now" value={liveBalance != null ? formatUSD(liveBalance) : "—"} />
        <Stat
          label="Wallet dropped (snapshots)"
          value={impliedSpend != null ? formatUSD(impliedSpend) : "—"}
          sub="from balance snapshots"
        />
        <Stat
          label="Carrier spend (billed)"
          value={formatUSD(w.telnyx_cost)}
          sub={`${w.segments.toLocaleString()} segments`}
          destructive
        />
        <Stat
          label="Wasted on non-delivery"
          value={formatUSD(w.wasted_cost)}
          sub={`${wastedPct.toFixed(0)}% of spend`}
          warn={wastedPct > 20}
        />
      </div>
      ...
      {rateRef && (
        <div className="text-xs text-muted-foreground mb-3 flex flex-wrap gap-x-4 gap-y-1">
          ...
          <span>
            = <b className="text-foreground">{rateRef.total_per_segment.toFixed(4)}/segment</b>
          </span>
...
function Stat({
  ...
  return (
    <div className={`rounded-lg border p-3 ${warn ? "border-amber-500/50 bg-amber-500/5" : ""}`}>
      <div className="text-xs uppercase tracking-wide text-muted-foreground">{label}</div>
      <div
        className={`text-lg font-semibold tabular-nums ${destructive ? "text-destructive" : ""} ${warn ? "text-amber-600" : ""}`}
      >
        {value}
      </div>
  ...
  return (
    <Card className="p-5">
      <h3 className="font-semibold mb-3 flex items-center gap-2">
        <TrendingDown className="w-4 h-4 text-primary" /> Per-tenant carrier spend (30d)
      </h3>
      <p className="text-xs text-muted-foreground mb-3">
        Pick a tenant to see exactly which carrier money their sending cost — and how much was
        wasted on messages that never delivered.
      </p>
      <select
      ...
      {!selectedId && (
        <div className="text-sm text-muted-foreground">
          Choose a tenant above to load their breakdown.
        </div>
      )}
      {selectedId && loading && (
        <div className="text-sm text-muted-foreground">Loading tenant spend…</div>
      )}
      ...
      {selectedId && data && (
        <div>
          <div className="grid md:grid-cols-4 gap-3 mb-4">
            <Stat
              label="Carrier spend"
              value={formatUSD(data.telnyx_cost)}
              sub={`${data.segments.toLocaleString()} segments`}
              destructive
            />
            <Stat
              label="Wasted on non-delivery"
              value={formatUSD(data.wasted_cost)}
              warn={data.telnyx_cost > 0 && (data.wasted_cost / data.telnyx_cost) * 100 > 20}
            />
          ...
          {rateRef && (
            <div className="text-xs text-muted-foreground mb-3">
              Rate ({rateRef.country}): {rateRef.total_per_segment.toFixed(4)}/segment · MMS ×
              {rateRef.mms_cost_multiplier}
            </div>
          )}
