    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Manage independent verifiers, their submitted numbers, sales, and payouts.
        </p>
      </div>
...
function VerifiersTab() {
  ...
  const toggleMut = useMutation({
    mutationFn: (a: { id: string; on: boolean }) =>
      setActive({ data: { verifier_id: a.id, is_active: a.on } }),
    onSuccess: () => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>All verifiers</CardTitle>
      </CardHeader>
      <CardContent>
        {(data ?? []).length === 0 ? (
          <div className="text-sm text-muted-foreground">No verifiers yet.</div>
        ) : (
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {v.email} · joined {new Date(v.created_at).toLocaleDateString()}
                    </div>
                    <div className="text-xs mt-1">
                      Wallet: <b>₦{Number(v.wallet?.balance_ngn ?? 0).toLocaleString()}</b> ·
                      Earned: ₦{Number(v.wallet?.lifetime_earned_ngn ?? 0).toLocaleString()}
                    </div>
                    {v.bank ? (
                      <div className="text-xs text-muted-foreground mt-1">
                        Bank: {v.bank.bank_name} · {v.bank.account_number} · {v.bank.account_name}
                      </div>
                    ) : (
                  ...
                  <Button
                    size="sm"
                    variant={v.is_active === false ? "default" : "outline"}
                    onClick={() => toggleMut.mutate({ id: v.id, on: v.is_active === false })}
                  >
                ...
                <div className="flex items-end gap-2 pt-2 border-t">
                  <div className="flex-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => setAmountMap({ ...amountMap, [v.id]: e.target.value })}
                    />
                  </div>
                  <div className="flex-1">
                    <Label className="text-xs">Reason</Label>
                    <Input
                      placeholder="Bonus / correction / clawback"
                      value={reasonMap[v.id] ?? ""}
                      onChange={(e) => setReasonMap({ ...reasonMap, [v.id]: e.target.value })}
                    />
                  </div>
                  <Button
                    size="sm"
                    disabled={!amountMap[v.id] || !reasonMap[v.id] || adjustMut.isPending}
                    onClick={() => {
...
function SubmissionsTab() {
  ...
  const qc = useQueryClient();
  const [status, setStatusFilter] = useState<"pending_verification" | "rejected" | "all">(
    "pending_verification",
  );
  const { data } = useQuery({
    queryKey: ["admin", "tfns", status],
  ...
  const setStatus = useMutation({
    ...
    onSuccess: () => {
      toast.success("Updated");
      qc.invalidateQueries({ queryKey: ["admin", "tfns"] });
      qc.invalidateQueries({ queryKey: ["admin", "verifiers"] });
    },
    onError: (e: any) => toast.error(e.message),
  });
              ...
              <div key={t.id} className="border rounded-md p-3 space-y-2">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {t.verifiers?.full_name} · {t.verifiers?.email}
                    </div>
                  ...
                  <div className="flex gap-2 shrink-0">
                    {t.status !== "verified" && (
                      <Button
                        size="sm"
                        onClick={() => setStatus.mutate({ id: t.id, status: "verified" })}
                      >
                        Approve
                      </Button>
                    )}
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setStatus.mutate({
                            id: t.id,
                            status: "rejected",
                            reason: reasonMap[t.id] || "Did not meet requirements",
                          })
                        }
                      >
                        Reject
                    ...
                    {t.status === "rejected" && (
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          setStatus.mutate({ id: t.id, status: "pending_verification" })
                        }
                      >
                        Reopen
                      </Button>
                ...
                {t.status !== "verified" && (
                  <Input
                    ...
                    value={reasonMap[t.id] || ""}
                    onChange={(e) => setReasonMap({ ...reasonMap, [t.id]: e.target.value })}
                  />
                )}
              </div>
            ))}
...
function SoldTab() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Sold numbers & payouts ({(data ?? []).length})</CardTitle>
      </CardHeader>
      <CardContent>
        {(data ?? []).length === 0 ? (
              ...
              <div key={s.id} className="border rounded-md p-3">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      Sold {s.sold_at ? new Date(s.sold_at).toLocaleString() : "—"}
                    </div>
                  </div>
                  <div className="text-xs">
                    <div>
                      <span className="text-muted-foreground">Buyer:</span>{" "}
                      {s.buyer?.email ?? s.sold_to_account_id}
                    </div>
                    <div>
                      <span className="text-muted-foreground">Verifier (payee):</span>{" "}
                      <b>{s.verifiers?.full_name}</b> · {s.verifiers?.email}
                    </div>
                  </div>
                  <div className="text-xs text-right">
                    <div>Sale: ₦{Number(s.sale_price_ngn ?? 0).toLocaleString()}</div>
                    <div>
                      Payout: <b>₦{Number(s.payout_ngn ?? 0).toLocaleString()}</b>
                    </div>
                    <div className="text-muted-foreground">
                      Commission: ₦{Number(s.commission_ngn ?? 0).toLocaleString()}
                    </div>
                  </div>
                </div>
              </div>
...
function CarrierApprovedTab() {
  ...
  const assign = useMutation({
    mutationFn: (args: { phone: string; account: string }) =>
      assignFn({ data: { phone_number: args.phone, account_id: args.account, country: "US" } }),
    onSuccess: () => {
      toast.success("Assigned to tenant");
      qc.invalidateQueries({ queryKey: ["admin", "carrier-tfns"] });
    },
    onError: (e: any) => toast.error(e.message),
  });
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-xs text-muted-foreground">
          Live list of carrier-approved verifications on your main upstream account. Assign any of
          them to a tenant.
        </p>
      </CardHeader>
      <CardContent>
        ...
        {error && (
          <div className="text-sm text-destructive">Upstream error: {(error as any).message}</div>
        )}
        {!isLoading && !error && (data ?? []).length === 0 && (
          <div className="text-sm text-muted-foreground">No approved toll-free numbers yet.</div>
        )}
        <div className="space-y-2">
          {(data ?? []).map((r: any) => (
            <div
            ...
            >
              <div className="min-w-0">
                <div className="font-mono">
                  {r.phone_number ?? <span className="text-muted-foreground">(no phone)</span>}
                </div>
                <div className="text-xs text-muted-foreground">
                ...
                {r.assigned_to && (
                  <div className="text-xs mt-1">
                    Assigned to <b>{r.assigned_to.email}</b>
                  </div>
                )}
              </div>
              <div className="flex items-center gap-2">
                {r.phone_number && !r.assigned_account_id && (
                  <>
                    <Select
                      value={assignMap[r.sid] || ""}
                      onValueChange={(v) => setAssignMap({ ...assignMap, [r.sid]: v })}
                    >
                    ...
                    <Button
                      size="sm"
                      disabled={!assignMap[r.sid] || assign.isPending}
                      onClick={() =>
                        assign.mutate({ phone: r.phone_number, account: assignMap[r.sid] })
                      }
                    >
                      Assign
                    </Button>
                ...
                {r.assigned_account_id && (
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      if (confirm(`Unassign ${r.phone_number}?`)) unassign.mutate(r.phone_number);
                    }}
                  >
                    Unassign
                  </Button>
...
function PoolTab() {
  ...
  const accountsFn = useServerFn(adminListAccountsLite);
  const qc = useQueryClient();
  const { data: tfns } = useQuery({
    queryKey: ["admin", "tfns", "verified"],
    queryFn: () => listFn({ data: { status: "verified" } }),
  });
  const { data: accounts } = useQuery({
    queryKey: ["admin", "accounts", "lite"],
    queryFn: () => accountsFn(),
  ...
  const assign = useMutation({
    mutationFn: (args: { tfn: string; account: string }) =>
      assignFn({ data: { tfn_id: args.tfn, account_id: args.account } }),
    onSuccess: () => {
      toast.success("Number assigned");
      qc.invalidateQueries({ queryKey: ["admin", "tfns", "verified"] });
    },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Verified pool ({(tfns ?? []).length})</CardTitle>
      </CardHeader>
      <CardContent>
        {(tfns ?? []).length === 0 ? (
          <div className="text-sm text-muted-foreground">Pool empty.</div>
        ) : (
          <div className="space-y-2">
            {(tfns ?? []).map((t: any) => (
              <div
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <Select
                    ...
                    onValueChange={(v) => setAssignMap({ ...assignMap, [t.id]: v })}
                  >
                    <SelectTrigger className="w-64">
                      <SelectValue placeholder="Assign to tenant…" />
                    </SelectTrigger>
                  ...
                  <Button
                    ...
                    onClick={() => assign.mutate({ tfn: t.id, account: assignMap[t.id] })}
                  >
                    Assign
                  </Button>
                  <Button
                    ...
                    variant="destructive"
                    onClick={() => {
                      if (confirm(`Remove ${t.phone_number} from pool?`)) del.mutate(t.id);
                    }}
                  >
                    Delete
                  </Button>
...
function WithdrawalsTab() {
  ...
  const reject = useMutation({
    mutationFn: (id: string) =>
      rejectFn({ data: { withdrawal_id: id, admin_note: noteMap[id] || "Rejected" } }),
    onSuccess: () => {
              ...
              <div key={w.id} className="border rounded-md p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {w.verifiers?.full_name} · {w.verifiers?.email}
                    </div>
                    {w.bank && (
                      <div className="text-xs mt-1">
                        → {w.bank.bank_name} · {w.bank.account_number} · {w.bank.account_name}
                      </div>
                    )}
                    <div className="text-xs text-muted-foreground mt-1">
                ...
                {w.status === "pending" && (
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => pay.mutate(w.id)}>
                        Mark paid
                      </Button>
                      <Button size="sm" variant="destructive" onClick={() => reject.mutate(w.id)}>
                        Reject
                      </Button>
                    </div>
                  </div>
                ...
                {w.admin_note && w.status !== "pending" && (
                  <div className="text-xs">Note: {w.admin_note}</div>
                )}
              </div>
            ))}
          </div>
    ...
    mutationFn: () =>
      setFn({
        data: {
          ...
          ngn_per_usd: Number(ngnRate),
        },
      }),
    onSuccess: () => {
      toast.success("Saved");
      qc.invalidateQueries({ queryKey: ["admin", "tfn-settings"] });
    },
    onError: (e: any) => toast.error(e.message),
  });
      ...
      <CardContent className="space-y-3 max-w-md">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Verifiers are paid in NGN; buyer's USD price is converted at this rate.
          </p>
        </div>
