...
import {
  getBalanceDropAudit,
  importTelnyxTransactionsCsv,
  listImportedTransactionBatches,
  deleteImportBatch,
} from "@/lib/admin-telnyx-audit.functions";
...
function BalanceDropAuditPage() {
  ...
  const audit = useQuery({
    ...
    queryFn: () =>
      auditFn({
        data: {
          start: toIso(new Date(start)),
          end: toIso(new Date(new Date(end).getTime() + 24 * 3600 * 1000 - 1)),
        },
      }),
  });
  const batches = useQuery({
    queryKey: ["admin", "telnyx-import-batches"],
    queryFn: () => listFn(),
  });
  ...
  async function handleFile(file: File) {
    ...
    if (rows.length === 0) {
      toast.error(
        "No rows detected. Expected columns: date/created_at, amount, category, description",
      );
      return;
    }
  ...
  const outflowSummary = useMemo(() => {
    ...
    return [
      {
        label: "SMS / MDR (derived)",
        amount: a.derived.sms_cost,
        note: `${a.derived.sms_messages.toLocaleString()} msgs · ${a.derived.sms_segments.toLocaleString()} segs`,
      },
      {
        label: "Toll-free verifications (derived)",
        amount: a.derived.verification_fees,
        note: `${a.derived.verifications} attempts × $75`,
      },
      {
        label: "Imported Telnyx debits",
        amount: a.imported.total_debits,
        note: `${(a.imported.rows as any[]).filter((r) => Number(r.amount) < 0).length} rows`,
      },
    ];
  }, [a]);
      ...
      sections: [
        {
          ...
          items: [
            [
              "First snapshot",
              a.snapshots.first
                ? `${formatUSD(Number(a.snapshots.first.balance))} on ${new Date(a.snapshots.first.checked_at).toLocaleString()}`
                : "—",
            ],
            [
              "Last snapshot",
              a.snapshots.last
                ? `${formatUSD(Number(a.snapshots.last.balance))} on ${new Date(a.snapshots.last.checked_at).toLocaleString()}`
                : "—",
            ],
            ["Balance delta", a.snapshots.delta === null ? "—" : formatUSD(a.snapshots.delta)],
            [
              "Observed outflow",
              a.snapshots.observed_outflow === null ? "—" : formatUSD(a.snapshots.observed_outflow),
            ],
          ],
        },
        {
          type: "kv",
          title: "Derived outflow",
          items: [
        ...
        {
          type: "kv",
          title: "Imported Telnyx transactions",
          items: [
        ...
        },
        {
          ...
          items: [
            [
              "Unexplained (observed − derived)",
              a.unexplained === null ? "—" : formatUSD(a.unexplained),
            ],
          ],
        },
        ...(a.imported.by_category.length > 0
          ? [
              {
                type: "table" as const,
                title: "By category (imported)",
                head: ["Category", "Rows", "Amount"],
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold flex items-center gap-2">
          <Scale className="size-6" /> Balance-drop audit
        </h1>
        <p className="text-sm text-muted-foreground">
          Explains what caused Telnyx's balance to move in the selected window. Combines derived
          SMS/MDR + verification fees with any Transactions CSV you upload.
        </p>
      </div>
      ...
      <Card className="p-4 flex flex-wrap gap-3 items-end">
        ...
        <div className="ml-auto flex gap-2">
          <Button variant="outline" onClick={exportAuditPdf} disabled={!a}>
      ...
      {audit.isLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <Stat
              label="Observed balance outflow"
              value={
                a.snapshots.observed_outflow === null
                  ? "—"
                  : formatUSD(a.snapshots.observed_outflow)
              }
              tone="warn"
            />
            <Stat
              label="Explained (derived + imported)"
              value={formatUSD(a.derived.total_derived_outflow + a.imported.total_debits)}
              tone="ok"
            />
            <Stat
              label="Unexplained gap"
              value={a.unexplained === null ? "—" : formatUSD(a.unexplained)}
              tone={a.unexplained !== null && Math.abs(a.unexplained) > 5 ? "bad" : "ok"}
            />
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <Card className="overflow-hidden">
              <div className="p-3 border-b bg-muted/40 font-semibold text-sm">
                SMS spend by sender kind
              </div>
              <table className="w-full text-sm">
                <thead className="text-left bg-muted/30">
                  <tr>
                    <th className="p-3">Kind</th>
                    <th className="p-3 text-right">Msgs</th>
                ...
                <tbody>
                  {a.derived.by_sender_kind.map((k) => (
                    <tr key={k.kind} className="border-t">
                      <td className="p-3 capitalize">{k.kind.replace("_", " ")}</td>
                      <td className="p-3 text-right tabular-nums">{k.count.toLocaleString()}</td>
                      <td className="p-3 text-right tabular-nums">{k.segments.toLocaleString()}</td>
                      <td className="p-3 text-right tabular-nums">{formatUSD(k.cost)}</td>
                    </tr>
                  ))}
                  {a.derived.by_sender_kind.length === 0 && (
                    <tr>
                      <td colSpan={4} className="p-6 text-center text-muted-foreground">
                        No SMS in this window.
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <Card className="overflow-hidden">
              <div className="p-3 border-b bg-muted/40 font-semibold text-sm">
                Imported transactions by category
              </div>
              <table className="w-full text-sm">
                <thead className="text-left bg-muted/30">
                  <tr>
                ...
                <tbody>
                  {a.imported.by_category.map((c) => (
                    <tr key={c.category} className="border-t">
                      ...
                      <td className="p-3 text-right tabular-nums">{formatUSD(c.amount)}</td>
                    </tr>
                  ))}
                  {a.imported.by_category.length === 0 && (
                    <tr>
                      <td colSpan={3} className="p-6 text-center text-muted-foreground">
                        Nothing imported for this window. Upload a Telnyx CSV below.
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      <Card className="p-4 space-y-3">
        <div className="font-semibold flex items-center gap-2">
          <Upload className="size-4" /> Import Telnyx Transactions CSV
        </div>
        <p className="text-xs text-muted-foreground">
          Export from Telnyx Portal → <b>Billing → Transactions</b> as CSV, then upload here. The
          importer maps common column names (<i>Date/Created At</i>, <i>Amount</i>,{" "}
          <i>Category/Type</i>, <i>Description</i>, <i>Reference/ID</i>).
        </p>
        <Input
          type="file"
          accept=".csv,text/csv"
                  ...
                  <tr key={b.batch_id} className="border-t">
                    ...
                    <td className="text-right">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          confirm("Delete this batch?") && deleteMut.mutate(b.batch_id)
                        }
                      >
