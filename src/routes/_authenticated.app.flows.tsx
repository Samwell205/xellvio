  ...
  head: () => ({
    meta: [
      { title: "Flows — SMS automation — Xellvio" },
      {
        name: "description",
        content:
          "Automate SMS follow-ups that send themselves when someone joins a list or texts a keyword.",
      },
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="max-w-2xl text-sm text-muted-foreground">
            Set up texts that send themselves — a welcome message when someone joins, or an instant
            reply when they text your keyword. You are only charged for texts that actually go out.
          </p>
        </div>
        ...
        <Card className="flex flex-col items-center gap-4 p-14 text-center">
          ...
          <div className="space-y-1">
            ...
            <p className="mx-auto max-w-md text-sm text-muted-foreground">
              Welcome new subscribers, answer keywords like JOIN, and follow up a few hours later —
              ...
              onClick={() =>
                setDraft({
                  ...
                  steps: [
                    {
                      delay_minutes: 0,
                      body: "You're in! Expect our best offers by text. Reply STOP to opt out.",
                    },
                  ],
                })
              }
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1">
                      ...
                      {TRIGGER_LABEL[f.trigger_type as Draft["trigger_type"]]}
                      {f.trigger_keyword ? `: ${f.trigger_keyword}` : ""}
                    </span>
                    <span className="flex items-center gap-1">
                      <MessageSquare className="size-3" />
                      {f.steps.length} message{f.steps.length === 1 ? "" : "s"}
                    </span>
                    <span>
                      {f.stats.sent} sent · {f.stats.scheduled} waiting · {f.stats.failed} failed
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(v) =>
                        toggle.mutate({ id: f.id, status: v ? "live" : "paused" })
                      }
                    />
                  </div>
                  <Button
            ...
            {runsQ.data!.slice(0, 10).map((r: any) => (
              ...
              >
                ...
                <span className="text-xs text-muted-foreground">
                  {new Date(r.sent_at ?? r.run_at).toLocaleString()}
                </span>
              </div>
            ))}
          </div>
      ...
      <Dialog open={!!draft} onOpenChange={(o) => !o && setDraft(null)}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Pick what starts it, then write the texts it should send.
            </DialogDescription>
          </DialogHeader>
          {draft && (
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setDraft({ ...draft, trigger_type: v as Draft["trigger_type"] })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                ...
                {draft.trigger_type === "list_join" && (
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setDraft({ ...draft, trigger_list_id: v === "any" ? null : v })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                ...
                {draft.trigger_type === "keyword_reply" && (
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setDraft({ ...draft, trigger_keyword: e.target.value.toUpperCase() })
                      }
                      placeholder="JOIN"
                      maxLength={40}
                    />
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setDraft({
                        ...draft,
                        steps: [...draft.steps, { delay_minutes: 60, body: "" }],
                      })
                    }
                  >
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      {draft.steps.length > 1 && (
                        <Button
                          ...
                          size="icon"
                          onClick={() =>
                            setDraft({ ...draft, steps: draft.steps.filter((_, x) => x !== i) })
                          }
                        >
                          <Trash2 className="size-4 text-destructive" />
                        </Button>
              ...
              <div className="flex items-center justify-between rounded-lg border p-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Live flows send automatically to new triggers.
                  </p>
                </div>
                <Switch
          ...
          <DialogFooter>
            ...
            <Button
              disabled={
                !draft?.name.trim() || !draft?.steps.every((s) => s.body.trim()) || save.isPending
              }
