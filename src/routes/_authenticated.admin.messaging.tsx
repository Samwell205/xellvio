...
function AdminMessagingPage() {
  ...
  const stats = useMemo(() => {
    ...
    const failed = rows.filter(
      (r: any) => r.status === "failed" || r.status === "undelivered",
    ).length;
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold flex items-center gap-2">
          <MessageSquareText className="size-6" /> Message monitor
        </h1>
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="p-4">
          <div className="text-xs uppercase text-muted-foreground">Sample</div>
          <div className="text-2xl font-extrabold">{stats.total}</div>
        </Card>
        <Card className="p-4">
          <div className="text-xs uppercase text-muted-foreground">Delivered</div>
        ...
        <Card className="p-4">
          ...
          <div className="text-2xl font-extrabold text-destructive">{stats.failed}</div>
        </Card>
      </div>
      ...
      <Card className="p-3 flex flex-wrap gap-2 items-center">
        <Input
          ...
          placeholder="Search number, tenant, body…"
          className="max-w-xs"
        />
              ...
              <tbody>
                {filtered.map((m: any) => (
                  <tr key={m.id} className="border-t align-top">
                    ...
                    <td className="p-3 truncate max-w-[160px] text-muted-foreground">
                      {m.campaign_name ?? "—"}
                    </td>
                    <td className="p-3 tabular-nums">{m.phone_e164}</td>
                    <td className="p-3 max-w-[320px]">
                      <div
                        className="text-xs whitespace-pre-wrap break-words line-clamp-3"
                        title={m.rendered_body ?? ""}
                      >
                    ...
                    <td className="p-3 text-right tabular-nums">
                      {m.cost ? formatUSD(Number(m.cost)) : "—"}
                    </td>
                    <td className="p-3">
                      <Badge
                        variant={
