...
function AudiencePage() {
  ...
  const profilesQ = useQuery({
    ...
    queryFn: async (): Promise<ProfileRow[]> =>
      listProfilesFn({ data: { listId: listFilter === "all" ? null : listFilter } }),
  });
  ...
  async function deleteAndOfferUndo(ids: string[]) {
    ...
    const { data: consentSnap } = await supabase
      .from("consents")
      .select("profile_id,channel,status,source,consented_at")
      .in("profile_id", ids);
    const { data: memSnap } = await sb
      .from("profile_list_members")
      .select("profile_id,list_id,account_id,added_at")
      .in("profile_id", ids);
        ...
        onClick: async () => {
          try {
            ...
            if (consentSnap?.length) {
              await supabase
                .from("consents")
                .upsert(consentSnap as any[], { onConflict: "profile_id,channel" });
            }
            if (memSnap?.length) {
              await sb
                .from("profile_list_members")
                .upsert(memSnap as any[], { onConflict: "list_id,profile_id" });
            }
  ...
  function downloadTemplate() {
    ...
    a.href = url;
    a.download = "xellvio-contacts-template.csv";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        ...
        <Stat
          icon={CheckCircle2}
          label="Subscribed (SMS)"
          value={statsQ.data?.subs ?? 0}
          tone="success"
        />
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-xs uppercase text-muted-foreground tracking-wide flex items-center gap-1">
          <ListIcon className="size-3.5" /> Lists:
        </span>
        <button
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  if (
                    confirm(
                      `Delete ${selected.size} contact${selected.size === 1 ? "" : "s"}? You'll have 8 seconds to undo.`,
                    )
                  ) {
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        <Button
                          ...
                          onClick={() => toggleConsent.mutate(r)}
                        >
...
function ManageListsDialog({ lists, onDone }: { lists: ContactList[]; onDone: () => void }) {
    ...
    try {
      ...
      if (editingId) {
        const { error } = await sb
          .from("contact_lists")
          .update({ name: name.trim(), description: desc || null })
          .eq("id", editingId);
  ...
  async function removeWithContacts(id: string, name: string) {
    if (!confirm(`Delete list "${name}" AND every contact inside it? This cannot be undone.`))
      return;
    setBusy(true);
    try {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Group contacts into named lists you can target with segments and campaigns.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          <div className="grid gap-2">
            ...
            <Input
              ...
              placeholder="e.g. Black Friday VIPs"
            />
            <Textarea
              value={desc}
                ...
                >
                  ...
                  <div className="flex gap-1">
                    ...
                    <Button
                      ...
                      title="Delete list only (keep contacts)"
                      onClick={() => remove(l.id)}
                    >
                      <Trash2 className="size-4" />
                    </Button>
                    <Button
                      ...
                      size="sm"
                      className="text-destructive"
                      title="Delete list AND all contacts in it"
                      disabled={busy}
                      onClick={() => removeWithContacts(l.id, l.name)}
                    >
                      <Trash2 className="size-4" />
                      <span className="ml-1 text-[10px] font-semibold">+ contacts</span>
                    </Button>
                  </div>
                </div>
              ))}
...
function AssignToListBulk({
  ...
  async function assign() {
    ...
    try {
      ...
      const { error } = await sb
        .from("profile_list_members")
        .upsert(rows, { onConflict: "list_id,profile_id" });
      if (error) throw error;
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Assign {ids.length} to a list</DialogTitle>
        </DialogHeader>
        {lists.length === 0 ? (
        ...
        <DialogFooter>
          ...
          <Button onClick={assign} disabled={busy || !listId}>
        ...
        .upsert(
          {
            ...
            last_name: last || null,
            country_code: parsed.country ?? country,
          },
          { onConflict: "account_id,phone_e164" },
        )
        .select("id")
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            You attest this person opted in to receive SMS from you.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-3">
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="col-span-2">
              <Label>Phone</Label>
              <Input
                placeholder="+15551234567 or local"
                value={phone}
