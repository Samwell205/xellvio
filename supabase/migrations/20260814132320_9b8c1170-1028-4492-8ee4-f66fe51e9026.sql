...
ADD COLUMN IF NOT EXISTS force_sms boolean NOT NULL DEFAULT false;

DROP FUNCTION IF EXISTS public.claim_campaign_messages(uuid, integer);
...
BEGIN
  ...
  FOR rec IN
    SELECT m.id,m.phone_e164,m.rendered_body,m.country_code,COALESCE(m.segments_count,1) AS segments_count,
      COALESCE(m.is_mms,false) AS is_mms,COALESCE(m.force_sms,false) AS force_sms,COALESCE(m.attempt_number,0) AS previous_attempt,
  ...
  LOOP
    ...
    END IF;
    profitable_price:=ROUND(rec.sell_price*rec.segments_count*CASE WHEN rec.is_mms AND NOT rec.force_sms THEN COALESCE(rec.mms_multiplier,1) ELSE 1 END,4);
    carrier_cost:=ROUND((COALESCE(rec.cost_price,0)+COALESCE(rec.passthrough_fee,0))*rec.segments_count*CASE WHEN rec.is_mms AND NOT rec.force_sms THEN COALESCE(rec.mms_cost_multiplier,rec.mms_multiplier,1) ELSE 1 END,6);
    ...
    VALUES(campaign_account,'debit',profitable_price,current_balance,_campaign_id,'Reserved '||CASE WHEN rec.force_sms THEN 'SMS fallback' ELSE 'SMS' END||' attempt '||next_attempt||' → '||rec.phone_e164||' ('||COALESCE(rec.country_code,'??')||') × '||rec.segments_count);
    ...
    VALUES(rec.id,_campaign_id,campaign_account,next_attempt,CASE WHEN next_attempt=1 THEN 'original_campaign' ELSE COALESCE(rec.retry_authorization_source,'manual_retry') END,CASE WHEN next_attempt=1 THEN NULL ELSE rec.retry_authorized_by END,CASE WHEN next_attempt=1 THEN 'Original campaign send' WHEN rec.force_sms THEN 'Explicitly approved retry as SMS without media' ELSE 'Explicitly approved retry' END,profitable_price,carrier_cost,'reserved');
    id:=rec.id; phone_e164:=rec.phone_e164; rendered_body:=rec.rendered_body; country_code:=rec.country_code; segments_count:=rec.segments_count; cost:=profitable_price; attempt_number:=next_attempt; force_sms:=rec.force_sms; RETURN NEXT;
  END LOOP;
END;$function$;
