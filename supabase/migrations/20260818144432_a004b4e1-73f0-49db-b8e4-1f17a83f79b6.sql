...
BEGIN
  -- Keyset cursor: highest profile_id already planned. NULL profile_id rows
  -- (e.g. retried/resent messages) must never reset the cursor, otherwise the
  -- whole audience looks unplanned forever and the campaign never finalizes.
    ...
    WITH page AS MATERIALIZED (
      ...
      WHERE plm.list_id = ANY(v_list_ids)
        ...
        AND NOT EXISTS (
          ...
          WHERE m.campaign_id = _campaign_id AND m.profile_id = p.id
        )
        AND NOT EXISTS (
...
UPDATE public.campaigns SET status='sent', paused_reason=NULL
