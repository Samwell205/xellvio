...
BEGIN
  ...
  -- Already-paid rows first. A row can be queued with charged_at set when a
  -- previous attempt was reserved but never handed to the carrier (worker
  -- interrupted, self-healing re-queue). Those must be re-dispatched WITHOUT
  -- charging again; skipping them left campaigns queued forever.
  RETURN QUERY
  WITH prepaid AS (
    SELECT m.id
     ...
     LIMIT LEAST(500, GREATEST(0,_limit))
     ...
     WHERE m.campaign_id=_campaign_id AND m.status='queued' AND m.charged_at IS NULL
       AND NOT EXISTS (
         SELECT 1 FROM public.messages recent
          WHERE recent.phone_e164 = m.phone_e164
            AND recent.created_at >= now()-interval '24 hours'
            AND recent.campaign_id <> m.campaign_id
          OFFSET 9 LIMIT 1
       )
  ...
  mark_ok AS (
    UPDATE public.messages m
       SET status='sending', dispatch_started_at=now(), cost=a.price, charged_at=now(),
           ...
           retry_authorization_source=NULL, retry_authorized_by=NULL, retry_authorized_at=NULL
      FROM affordable a
     WHERE m.id = a.id
    RETURNING m.id
  ),
  ...
  mark_bad AS (
    UPDATE public.messages m
       SET status='failed', error_code=c.verdict,
           failure_reason = CASE WHEN c.verdict='rate_unavailable'
             THEN 'No active price is available for this destination.'
  ...
  -- Frequency-capped rows were previously skipped silently, which left them
  -- queued forever and campaigns permanently "sending". Resolve them.
  UPDATE public.messages m
     SET status='failed', error_code='frequency_cap',
         failure_reason='Recipient already received the maximum number of messages allowed in 24 hours.'
   WHERE m.campaign_id=_campaign_id AND m.status='queued' AND m.charged_at IS NULL
     AND EXISTS (
       SELECT 1 FROM public.messages recent
        WHERE recent.phone_e164 = m.phone_e164
