...
BEGIN
  ...
  FOR rec IN
    ...
    LIMIT LEAST(480, GREATEST(0, _limit))
  LOOP
...
SELECT cron.unschedule(jobid) FROM cron.job WHERE jobname IN ('dispatch-campaigns-fast-15s','dispatch-campaigns-fast-30s','dispatch-campaigns-fast-45s');

SELECT cron.schedule(
  'dispatch-campaigns-fast-15s',
  '* * * * *',
  $cron$SELECT pg_sleep(15); SELECT net.http_post(url := 'https://project--91d3bf8a-0d22-4b7d-9569-057a8306639a.lovable.app/api/public/dispatch-campaign', headers := jsonb_build_object('Content-Type','application/json','apikey','eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRieXFrdGZlY2ZidWtnbGNpaWhjIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE3ODY5OTYsImV4cCI6MjA5NzM2Mjk5Nn0.IijlbZkJPlNvjp0_be_JRBYjrNwJmdWpte51rSSFcjw'), body := '{}'::jsonb, timeout_milliseconds := 60000);$cron$
);
SELECT cron.schedule(
  'dispatch-campaigns-fast-30s',
  '* * * * *',
  $cron$SELECT pg_sleep(30); SELECT net.http_post(url := 'https://project--91d3bf8a-0d22-4b7d-9569-057a8306639a.lovable.app/api/public/dispatch-campaign', headers := jsonb_build_object('Content-Type','application/json','apikey','eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhcGFzZSIsInJlZiI6ImRieXFrdGZlY2ZidWtnbGNpaWhjIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE3ODY5OTYsImV4cCI6MjA5NzM2Mjk5Nn0.IijlbZkJPlNvjp0_be_JRBYjrNwJmdWpte51rSSFcjw'), body := '{}'::jsonb, timeout_milliseconds := 60000);$cron$
);
SELECT cron.schedule(
  'dispatch-campaigns-fast-45s',
  '* * * * *',
  $cron$SELECT pg_sleep(45); SELECT net.http_post(url := 'https://project--91d3bf8a-0d22-4b7d-9569-057a8306639a.lovable.app/api/public/dispatch-campaign', headers := jsonb_build_object('Content-Type','application/json','apikey','eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhcGFzZSIsInJlZiI6ImRieXFrdGZlY2ZidWtnbGNpaWhjIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE3ODY5OTYsImV4cCI6MjA5NzM2Mjk5Nn0.IijlbZkJPlNvjp0_be_JRBYjrNwJmdWpte51rSSFcjw'), body := '{}'::jsonb, timeout_milliseconds := 60000);$cron$
);