...
BEGIN
  -- Keyset cursor. Planning always walks the audience in ascending profile_id
  -- order, so the highest profile_id already planned is a safe resume point.
  -- Without it every call re-scanned and re-sorted the entire audience (100k+
  -- rows) just to skip the part already planned, which hit the statement
  -- timeout and stalled planning permanently.
  SELECT max(m.profile_id) INTO v_after
  FROM public.messages m WHERE m.campaign_id = _campaign_id;
