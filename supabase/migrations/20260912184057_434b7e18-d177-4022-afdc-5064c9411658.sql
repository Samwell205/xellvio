...
BEGIN
  ...
  -- A worker may be interrupted after reserving a row but before recording the
  -- carrier response. Requeue the same paid attempt rather than permanently
  -- failing it. The dispatcher reuses the same idempotency key, so a request
  -- already accepted by the carrier cannot be delivered or charged twice.
